...
struct pos // position in the file
{
    int line;
    int col;
    const char* filename;
...
struct token
{
    ...
    // True if their is whitespce between the token and the next token
...
{
    struct pos pos;
    struct vector* token_vec; // vector of tokens(we have many tokens in the a program)
    struct compile_process* compiler;

    /*
        ((50)) 
        if we pass 50 as token,then it will be an expression 
        of two because we have two of those brackets
    */
...
{
    //The flags in regards how this file should be complied
    int flags;
